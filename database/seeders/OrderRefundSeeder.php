...
class OrderRefundSeeder extends Seeder
{
    /**
    ...
    {
        // Find completed orders with bank payment method
        $orders = Order::where('status_id', 2)
            ->where('payment_method', 'bank')
            ->get();

        if ($orders->isEmpty()) {
            $this->command->info('No completed bank payment orders found. Creating a test order...');

            // Check if we have any bank payment orders to update to completed
            $bankOrders = Order::where('payment_method', 'bank')->get();

            if (!$bankOrders->isEmpty()) {
                $order = $bankOrders->first();
                $order->status_id = 2; // Set to completed
                $order->save();

                $this->command->info("Updated order {$order->order_number} to completed status.");
            } else {
