...
class ProductController extends BaseController
{
    ...
    {
        try {
            // Track if this is a Dropzone upload with images
            $hasImages = $request->hasFile('images');
            $isProcessingImages = $request->header('X-Processing-Images');

            // Debug log for tracking request types
            Log::info('Store Product Request:', [
                'has_images' => $hasImages,
                'is_ajax' => $request->ajax(),
                'is_processing_images' => $isProcessingImages
            ]);

            // If this is a form with images, prevent standard form submission
            // Images will be handled by a separate request from Dropzone
            if ($hasImages && $request->has('prevent_create') && $request->input('prevent_create') === 'true') {
                Log::info('Received form submit with images, skipping product creation to let Dropzone handle it');
                return response()->json(['message' => 'Waiting for Dropzone to handle image uploads']);
            }

            $validated = $request->validate($this->model::rules());
            $validated['sku'] = $this->generateSKU($validated['name']);

            // Force ensuring slug uniqueness
            if (empty($validated['name'])) {
                throw new \Exception("Product name cannot be empty");
            }

            // IMPORTANT: Hardcode unique slug with timestamp + random string to avoid duplicates
            $baseSlug = Str::slug($validated['name']);
            if (empty($baseSlug)) {
                $baseSlug = 'product';
            }

            $uuid = (string) Str::uuid(); // Ensure UUID is a string
            $timestamp = time();
            $uniqueString = $timestamp . '-' . substr($uuid, 0, 8);
            $validated['slug'] = $baseSlug . '-' . $uniqueString;

            // Log generated slug for debugging
            Log::info('Creating product with slug: ' . $validated['slug']);

            // Ensure slug is unique with an extra check - this is a critical step
            $attempt = 0;
            $originalSlug = $validated['slug'];

            while (Product::where('slug', $validated['slug'])->exists() && $attempt < 5) {
                $attempt++;
                $validated['slug'] = $originalSlug . '-retry' . $attempt;
                Log::warning("Duplicate slug detected! Retry {$attempt} with new slug: {$validated['slug']}");
            }

            if ($attempt >= 5) {
                throw new \Exception("Could not generate a unique slug after 5 attempts");
            }

            // Begin transaction
            DB::beginTransaction();

            // Create the product with our custom pre-generated slug
    ...
    public function update(Request $request, $id)
    {
        ...
        // Only generate new slug if name changed
        if (($item->name != $validated['name']) || empty($item->slug)) {
            // Generate guaranteed unique slug
            $baseSlug = Str::slug($validated['name']);
            $uuid = (string) Str::uuid();
            $validated['slug'] = $baseSlug . '-' . time() . '-' . $uuid;

            // Extra check
            if (Product::where('slug', $validated['slug'])->where('id', '!=', $id)->exists()) {
                $validated['slug'] = $baseSlug . '-' . time() . '-' . $uuid . '-' . Str::random(8);
            }

            Log::info('Product update: using newly generated slug ' . $validated['slug']);
        } else {
            // Keep existing slug if name hasn't changed
            $validated['slug'] = $item->slug;
            Log::info('Product update: keeping existing slug ' . $validated['slug']);
        }
