...
<div class="body_content">
    ...
    <!-- Add this where you want the search bar (e.g., below header or banner) -->
    <div class="search-section my-4">
        <form action="{{ route('client.search') }}" method="GET" class="d-flex align-items-center justify-content-center">
    ...
    <div class="container mt-5">
        <div class="col-12 mb-5">
            <div class="section-head text-center">
                <h2 class="section-title">Siêu giảm giá</h2>
            </div>
        </div>
        <div class="row">
                                ...
                                <div class="product-card-l">
                                    ...
                                    <div class="product-title text-center py-2">
                                        ...
                                        <del class="text-muted">{{ number_format($minPrice, 0, ',', '.') }}
                                            đ</del>
                                        <span
                                            class="text-danger">{{ number_format($minSalePrice, 0, ',', '.') }}
                                            đ</span>
                                        @else
                                        <span
        ...
        <div class="container-fluid">
            <div class="container mt-5">
                ...
                <div class="row">
                    ...
                    @php
                    $image = $hotProduct->images->first()->url ?? '/client/assets/images/default.png';
                    $variation = $hotProduct->variations->first();
                    ...
                    <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4"> <!-- 4 sản phẩm trên 1 hàng -->
                        <div class="product-card-l">
                            <div class="product-img position-relative overflow-hidden">
                                <a href="{{ route('client.product.product-details', $hotProduct->id) }}" class="d-block">
                                    <!-- Ảnh chính: đồng bộ kích thước bằng ratio và object-fit -->
                                    <div class="ratio ratio-1x1">
                                        <img src="{{ asset($image) }}" alt="{{ $hotProduct->name }}"
                                            class="img-fluid w-100 object-fit-cover"
                                            style="transition: transform 0.3s ease;"
                                            onmouseover="this.style.transform='scale(1.1)'"
                                            onmouseout="this.style.transform='scale(1)'" />
                                ...
                                <div class="product-actions position-absolute bottom-0 end-0 p-2">
                                    ...
                                    <button class="btn p-0 border-0 shadow-none bg-transparent wishlist-toggle mb-1"
                                        ...
                                        <i class="bi bi-heart{{ auth()->check() && $hotProduct->wishlists()->where('user_id', auth()->id())->exists() ? '-fill text-danger' : '' }}"></i>
                                    </button>
                                    <a href="{{ route('client.product.product-details', $hotProduct->id) }}"
                                        class="text-dark me-2"><i class="flaticon-search"></i></a>
                                </div>
                            </div>
                            <div class="product-title text-center py-2">
                                <h3 class="product-title mb-2">
                                    <a href="{{ route('client.product.product-details', $hotProduct->id) }}"
                                        class="text-dark text-decoration-none link-primary">{{ $hotProduct->name }}</a>
                                </h3>
                                <div>
