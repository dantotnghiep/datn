                    ...
                    <div class="col-12 col-lg-6">
                        <div class="d-flex flex-column justify-content-between h-100">
                            <div>
                                ...
                                <div class="d-flex flex-wrap align-items-center">
                                    @if ($product->variations->count() > 0)
                                        ...
                                        @if ($variation->sale_price)
                                            ...
                                            @php
                                                $discount = round(
                                                    (($variation->price - $variation->sale_price) / $variation->price) *
                                                        100,
                                                );
                                            @endphp
                                            <p class="text-warning fw-bolder fs-6 mb-0">{{ $discount }}% giảm</p>
                                        @endif
                                    @else
                                ...
                                @if ($product->variations->count() > 0)
                                    @if ($variation->stock > 0)
                                        <p class="text-success fw-semibold fs-7 mb-2">Còn hàng ({{ $variation->stock }})
                                        </p>
                                    @else
                            ...
                            <div>
                                <div class="mb-3">
                                    @if($product->variations->count() > 0)
                                        <p class="fw-semibold mb-2 text-body">Phiên bản sản phẩm:</p>
                                            ...
                                            <select class="form-select w-auto" id="variation-select">
                                                @foreach($product->variations as $variation)
                                                    <option value="{{ $variation->id }}" 
                                                        ...
                                                        {{ $variation->name ? $variation->name : 'Mặc định' }} 
                                                        - {{ number_format($variation->sale_price ?: $variation->price) }}đ
                                                        {{ $variation->stock > 0 ? '(Còn ' . $variation->stock . ')' : '(Hết hàng)' }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="stock-status mt-2">
                                            @if($selectedVariation->stock > 0)
                                                <p class="text-success mb-0">
                                                    <i class="fas fa-check-circle"></i> Còn hàng ({{ $selectedVariation->stock }})
                                                </p>
                                            @else
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <input type="number" class="form-control form-control-sm mx-2 text-center" 
                                                       ...
                                                       @if(!$product->variations->count() || $selectedVariation->stock <= 0) disabled @endif>
    ...
    <script>
        $(document).ready(function() {
            $('#variation-select').change(function() {
                ...
                const salePrice = selected.data('sale-price');
                ...
                if (salePrice) {
                    ...
                    $('.price-display').html(`
                    <h1 class="me-3">${numberFormat(salePrice)}đ</h1>
                    <p class="text-body-quaternary text-decoration-line-through fs-6 mb-0 me-3">
                ...
                `);
                    ...
                    $('.price-display').html(`<h1 class="me-3">${numberFormat(price)}đ</h1>`);
                }

                // Cập nhật trạng thái tồn kho
                if (stock > 0) {
                    $('.stock-status').html(
                        `<p class="text-success fw-semibold fs-7 mb-2">Còn hàng (${stock})</p>`);
                } else {
                    $('.stock-status').html(`<p class="text-danger fw-semibold fs-7 mb-2">Hết hàng</p>`);
                }
            });
        });
