                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-10">
                                <div class="main-product-image border border-translucent rounded-3 text-center p-3 h-100">
                                    ...
                                    @endphp
                                    <img id="mainProductImage" src="{{ asset('storage/' . $primaryImage->image_path) }}"
                            ...
                            <div>
                                ...
                                @if ($product->is_hot)
                                    <div class="d-flex flex-wrap align-items-start mb-3">
                                        <span class="badge text-bg-danger fs-10 product-verified-badge">HOT<span
                                                class="fas fa-fire ms-1"></span></span>
                                    </div>
                                @endif
                                        ...
                                        <div class="price-display">
                                            @if ($selectedVariation->sale_price)
                                                <div class="d-flex align-items-center">
                                                    <h3 class="me-3 mb-0">
                                                        {{ number_format($selectedVariation->sale_price) }}đ</h3>
                                                    ...
                                                    @foreach ($attribute['values'] as $value)
                                                        <div class="form-check me-3">
                                                            <input class="form-check-input variation-attribute"
                                                                type="radio" name="attribute_{{ $attribute['id'] }}"
                                                                value="{{ $value['id'] }}"
                                        ...
                                        <div class="variation-quantity mb-4">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="ms-4">
                                                    ...
                                                    <span class="stock-status">{{ $selectedVariation->stock }} sản
                                                        phẩm</span>
                                                </div>
                                            </div>
                                        </div>
    ...
    <section class="py-0 mb-9">
        <div class="container">
            <div class="d-flex flex-between-center mb-3">
                ...
                <a href="{{ route('product.index', ['category' => $product->category_id]) }}"
                    class="btn btn-sm btn-phoenix-primary">Xem tất cả</a>
            </div>
                        ...
                        @foreach ($relatedProducts as $relatedProduct)
                            <div class="swiper-slide">
                                <div class="position-relative text-decoration-none product-card h-100">
                                    <div>
                                        <div class="border border-1 border-translucent rounded-3 position-relative mb-1">
                                            @if ($relatedProduct->first_variation_id)
                                                <button class="btn btn-wish btn-wish-primary z-2 d-toggle-container wishlist-btn"
                                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                                    data-variation-id="{{ $relatedProduct->first_variation_id }}"
                                                    title="{{ in_array($relatedProduct->first_variation_id, $wishlistItems ?? []) ? 'Đã yêu thích' : 'Thêm vào yêu thích' }}">
                                                    @if (in_array($relatedProduct->first_variation_id, $wishlistItems ?? []))
                                            ...
                                            @endif
                                            <img class="img-fluid" src="{{ $relatedProduct->first_image }}"
                                        ...
                                        <a href="{{ route('product.detail', $relatedProduct->slug) }}" class="text-decoration-none stretched-link">
                                            <h6 class="mb-0 lh-sm line-clamp-2 product-name">
                                                {{ $relatedProduct->name }}
                                            </h6>
                                        </a>
                                            ...
                                            @if($relatedProduct->has_sale)
                                                <div class="d-flex align-items-center">
                                                    <p class="me-2 text-body text-decoration-line-through mb-0">
                                                        {{ number_format($relatedProduct->regular_min_price, 0, ',', '.') }} -
                                                        {{ number_format($relatedProduct->max_price, 0, ',', '.') }}
                                                    </p>
                                                    <h6 class="text-danger mb-0 fw-bold">
                                                        {{ number_format($relatedProduct->min_price, 0, ',', '.') }} -
                                                        {{ number_format($relatedProduct->max_price, 0, ',', '.') }}
                                                    </h6>
                                                </div>
                                            @else
                                        ...
                                        @if ($relatedProduct->variations_count > 0)
                                            <p class="text-body-tertiary fw-semibold fs-9 lh-1 mb-0">
                                                {{ $relatedProduct->variations_count }} loại sản phẩm
                                            </p>
                                        @endif
                                    </div>
                                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateUI(variation) {
                ...
                if (variation.sale_price) {
                    const discount = variation.price > 0 ? Math.round(((variation.price - variation.sale_price) /
                        variation.price) * 100) : 0;
                        ...
                        if (data.status === 'success') {
                            ...
                            if (data.action === 'added') {
                                ...
                                addToWishlistBtn.innerHTML =
                                    `<span class="me-2 fas fa-heart text-warning" id="wishlist-icon"></span>Đã thêm vào yêu thích`;
                            } else {
