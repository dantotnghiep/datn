                ...
                <div class="d-flex flex-wrap gap-3">
                    <div class="search-box">
                        <form action="{{ route($route.'.index') }}" method="GET" class="position-relative">
                            ...
                            <input class="form-control search-input search" type="search" name="search" value="{{ request()->get('search') }}" 
                        ...
                        <div class="btn-group position-static" role="group">
                            @foreach($fields as $field => $options)
                                ...
                                <div class="btn-group position-static text-nowrap">
                                    <button class="btn btn-phoenix-secondary px-4 flex-shrink-0" type="button" data-bs-toggle="dropdown" 
                                            data-boundary="window" aria-haspopup="true" aria-expanded="false">
                                        {{ $options['label'] ?? ucfirst($field) }}
                                        <span class="fas fa-angle-down ms-2"></span>
                                    </button>
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="{{ route($route.'.index', ['trashed' => request()->get('trashed', 0)]) }}">All</a></li>
                            ...
                            <div class="btn-group position-static text-nowrap">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="{{ route($route.'.index', ['trashed' => request()->get('trashed', 0)]) }}">Default</a></li>
                                    @foreach($fields as $field => $options)
                        ...
                        <tbody class="list">
                            ...
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    @if($item->trashed())
                                        <form action="{{ route($route.'.restore', $item->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn btn-sm p-0 text-success me-2" title="Restore">
                                    ...
                                    <ul class="pagination mb-0">
                                        ...
                                        @if ($items->hasPages())
                                            ...
                                            @if ($currentPage < $lastPage - $window - 1)
                                                ...
                                                @endif
                                                <li class="page-item">
                                                    <a class="page-link" href="{{ $items->url($lastPage) }}">{{ $lastPage }}</a>
                                                </li>
                                            @endif
                                        @endif
