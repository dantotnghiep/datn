...
<div class="content">
    ...
    <div class="mb-9">
      ...
      <div class="d-sm-flex flex-between-center mb-3">
        <p class="text-body-secondary lh-sm mb-0 mt-2 mt-sm-0">Customer ID: <a class="fw-bold" href="#!">{{ $order->user_id }}</a></p>
        <div class="d-flex">
          ...
          @if ($orderStatus != 4 && $orderStatus != 5)
            <form action="{{ route($route . '.update-status', $order->id) }}" method="POST" class="d-inline ms-1">
                @csrf
                @method('PUT')
                <input type="hidden" name="status_id" value="5">
                <button type="submit" class="btn btn-link px-3 text-body" onclick="return confirm('Are you sure you want to refund this order?')">
                    ...
                    <td class="align-middle white-space-nowrap py-2">
                      @if($item->productVariation && $item->productVariation->product)
                        <a class="d-block border border-translucent rounded-2" href="#">
                          <img src="{{ $item->productVariation->product->first_image ?? asset('theme/prium.github.io/phoenix/v1.22.0/assets/img/products/1.png') }}" alt="{{ $item->productVariation->product->name }}" width="53" />
                        </a>
                      @else
                        <span class="d-block border border-translucent rounded-2 text-center p-2">No image</span>
                      @endif
                    </td>
                    <td class="size align-middle white-space-nowrap text-body-tertiary fw-semibold py-0 ps-4">
            ...
            <div class="col-12 col-sm-auto">
              ...
              <div class="row g-4 flex-sm-column">
                ...
                <div class="col-6 col-sm-12">
                  ...
                  </div><a class="d-block fs-9 ms-4" href="mailto:{{ $order->user->email ?? 'N/A' }}">{{ $order->user->email ?? 'N/A' }}</a>
                </div>
                <div class="col-6 col-sm-12 order-sm-1">
                  ...
                  <div class="ms-4">
                    ...
                    <p class="text-body-secondary mb-0 fs-9">{{ $order->address }}<br class="d-none d-sm-block" />{{ $order->ward }}, {{ $order->district }}, {{ $order->province }}</p>
                  </div>
                </div>
                <div class="col-6 col-sm-12">
                  <div class="d-flex align-items-center mb-1"><span class="me-2" data-feather="phone" style="stroke-width:2.5;">  </span>
                    <h6 class="mb-0">Phone</h6>
                  </div><a class="d-block fs-9 ms-4" href="tel:{{ $order->user_phone }}">{{ $order->user_phone }}</a>
                </div>
              </div>
            </div>
            <div class="col-12 col-sm-auto">
              ...
              <div class="row g-4 flex-sm-column">
                ...
                <div class="col-6 col-sm-12">
                  ...
                  <p class="mb-0 text-body-secondary fs-9 ms-4">{{ $order->created_at->format('d M, Y') }}</p>
                </div>
              </div>
            </div>
            <div class="col-12 col-sm-auto">
              ...
              <div class="row g-4 flex-sm-column">
                ...
                <div class="col-6 col-sm-12">
                  ...
                  <p class="mb-0 text-body-secondary fs-9 ms-4">{{ $order->notes ?? 'No notes provided' }}</p>
                </div>
                @if($order->paid_at)
                <div class="col-6 col-sm-12">
                  <div class="d-flex align-items-center mb-1"><span class="me-2" data-feather="calendar" style="stroke-width:2.5;">  </span>
                  ...
                  <p class="mb-0 text-body-secondary fs-9 ms-4">{{ $order->paid_at->format('d M, Y') }}</p>
                </div>
                @endif
              </div>
            </div>
                ...
                <div class="card-body">
                  ...
                  <div>
                    <div class="d-flex justify-content-between">
                      ...
                      <p class="text-body-emphasis fw-semibold">{{ number_format($order->total, 2) }}</p>
                    </div>
                    <div class="d-flex justify-content-between">
                      <p class="text-body fw-semibold">Discount :</p>
                      <p class="text-danger fw-semibold">-{{ number_format($order->discount, 2) }}</p>
                    </div>
                    <div class="d-flex justify-content-between">
                    ...
                    <div class="d-flex justify-content-between">
                      ...
                      <p class="text-body-emphasis fw-semibold">{{ number_format($order->total_with_discount, 2) }}</p>
                    </div>
                    <div class="d-flex justify-content-between">
